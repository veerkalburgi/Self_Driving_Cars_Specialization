...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    p_check = p_check + p_del
    v_check = v_check + v_del
    q_check = Quaternion(euler=phi_del).quat_mult_right(q_check)
    # 3.4 Compute corrected covariance
    p_cov_check = (np.identity(9) - K.dot(h_jac)).dot(p_cov_check)
